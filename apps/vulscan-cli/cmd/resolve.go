...
type projectContext struct {
	...
	ScanDir  string // output_root: ~/.vulscan/projects/org/repo/scans/{sha}/{lang}/ (runs under runs/<run_id>/)
	RepoPath string
	Language string
}
...
func resolveProject() (*projectContext, error) {
	...
	if projectFlag != "" {
		// The flag value becomes path segments under ~/.vulscan/projects/.
		if err := config.ValidateProjectName(projectFlag); err != nil {
			return nil, err
		}
