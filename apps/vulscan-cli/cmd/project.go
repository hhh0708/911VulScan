...
Examples:
  vulscan project list       List all projects
  vulscan project show       Show active project details
  vulscan project switch grafana/grafana   Switch active project`,
}
...
func runProjectList(cmd *cobra.Command, args []string) {
	...
	if len(names) == 0 {
		fmt.Println("No projects initialized. Run: vulscan init <repo-url-or-path> -l <language>")
		return
	}
