...
func ActiveProject() (*Project, error) {
	...
	if cfg.ActiveProject == "" {
		return nil, fmt.Errorf("no active project. Run: vulscan init <repo-url-or-path> -l <language>")
	}
...
//   - ~/.vulscan/projects/<name>/project.json         → local projects
//   - ~/.vulscan/projects/<org>/<repo>/project.json   → remote (org/repo) projects
//
...
func ListProjects() ([]string, error) {
	...
	// Read first-level directories (e.g. "grafana", "ghostty-org", "vulscan")
	...
	for _, l1 := range level1Entries {
		...
		// Check for project.json at level 1 (local projects: "vulscan")
