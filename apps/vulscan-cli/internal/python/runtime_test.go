...
func TestVenvPython_Windows(t *testing.T) {
	...
	vp := venvPython()
	expected := filepath.Join(os.Getenv("USERPROFILE"), ".vulscan", "venv", "Scripts", "python.exe")
	if vp != expected {
...
// a path under the user's real ~/.vulscan/venv/. The tests exercise the
...
func TestReadStoredHash_DoesNotPanic(t *testing.T) {
	// Smoke test: reading from the real ~/.vulscan/venv/.deps-hash must
