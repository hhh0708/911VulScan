...
func DetectViaPython(pythonPath, scanDir, stepName string) *Info {
	...
	if err != nil || !types.EnvelopeSucceeded(result.Envelope.Status) {
...
// Uses go-isatty (matching cmd/mode.go) — a naive os.ModeCharDevice check
// would falsely treat /dev/null as a TTY.
