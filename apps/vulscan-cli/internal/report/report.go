...
func init() {
	funcMap := template.FuncMap{
		// toJSON embeds chart data in <script> blocks. json.Marshal escapes
		// '<', '>' and '&' by default (SetEscapeHTML(true)); that default is
		// load-bearing here — do not disable it, or chart labels could break
...
// RenderReskin renders the light-themed HTML report to the given writer.
func RenderReskin(data ReportData, w io.Writer) error {
	return reskinTmpl.Execute(w, data)
}

// GenerateReskin renders the light-themed HTML report to a file.
