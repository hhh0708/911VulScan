...
func baseReportData() ReportData {
	return ReportData{
		...
		Language:  "python",
		Stats:     Stats{TotalUnits: 8, TotalFiles: 1, Reproduced: 2, Confirmed: 3, Candidates: 1, Rejected: 2},
		Categories: []Category{
			{Verdict: "Dynamically reproduced", Color: "#dc3545", Description: "x"},
		},
	}
}
...
func TestSafeRemediationEscapesHTML(t *testing.T) {
	d := ReportData{RemediationHTML: "<script>alert(1)</script>\nSecond line"}
	got := string(d.SafeRemediation())
	if strings.Contains(got, "<script>") {
		t.Errorf("SafeRemediation must escape raw HTML, got: %s", got)
	}
	if !strings.Contains(got, "&lt;script&gt;") {
		t.Errorf("expected escaped script tag, got: %s", got)
	}
	if !strings.Contains(got, "<br>") {
		t.Errorf("expected newline converted to <br>, got: %s", got)
	}
}
