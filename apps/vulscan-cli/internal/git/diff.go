...
// ref (e.g. "origin/main").
//
// The working tree is intentionally left on pr-head on success: the diff
// manifest (base...HEAD) and the Python pipeline both read the working
// tree, so restoring the previous branch must happen after the whole
// pipeline finishes (see cmd/diff_shared.go). On any failure the previous
// branch/HEAD is restored before returning.
func FetchPR(repoPath string, prNumber int, stderr *bytes.Buffer) (baseRef string, err error) {
	...
	if err := ValidateRef(meta.BaseRefName); err != nil {
		return "", fmt.Errorf("PR %d base ref: %w", prNumber, err)
	}

	// Record the current branch (or detached HEAD commit) so a failed
	// fetch/checkout never strands the user's working copy half-switched.
	prevRef := CurrentBranch(repoPath)
	if prevRef == "" {
		if sha, shaErr := gitRevParse(repoPath, "HEAD"); shaErr == nil {
			prevRef = sha
		}
	}
	defer func() {
		if err != nil && prevRef != "" {
			_ = runGit(repoPath, stderr, "checkout", prevRef)
		}
	}()
...
func runGit(repoPath string, stderr *bytes.Buffer, args ...string) error {
	...
	// Always capture stderr locally as well — the caller-provided buffer may
	// be discarded, but the error message must carry git's own output.
	var localErr bytes.Buffer
	if stderr != nil {
		cmd.Stderr = io.MultiWriter(stderr, &localErr)
	} else {
		cmd.Stderr = &localErr
	}
