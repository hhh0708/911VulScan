...
#   - race guard against public/$PUBLIC_BRANCH moving since the branch was cut
...
# Public remote branch to guard against (override via env if it is not master).
...
if [ "$NO_FETCH" = "0" ]; then
  if git remote get-url public >/dev/null 2>&1; then
    ...
    fail "remote 'public' missing — run: git remote add public git@github.com:hhh0708/911VulScan.git"
  fi
else
...
if [ "$POST_MERGE" = "1" ]; then
  ...
  section "Race guard vs public/$PUBLIC_BRANCH"

  if git rev-parse --verify --quiet "refs/remotes/public/$PUBLIC_BRANCH" >/dev/null; then
    AHEAD=$(git log --oneline "public/$PUBLIC_BRANCH" ^HEAD 2>/dev/null)
    if [ -n "$AHEAD" ]; then
      fail "public/$PUBLIC_BRANCH has commits not reachable from $CURRENT_BRANCH:"
      printf '%s\n' "$AHEAD" | sed 's/^/        /'
      echo "        rebase onto public/$PUBLIC_BRANCH before releasing"
    else
      pass "public/$PUBLIC_BRANCH fully reachable from $CURRENT_BRANCH"
    fi
  else
    fail "public/$PUBLIC_BRANCH ref not found — fetch the public remote"
  fi
fi
...
# ---------- Forbidden strings in commit history ----------

section "Forbidden strings in commit messages"

# Internal ticket IDs etc. must not leak through git history either.
if [ -f "$FORBID_FILE" ]; then
  any_history_failure=0
  COMMIT_MSGS=$(git log --format=%B)
  while IFS= read -r needle; do
    [ -z "$needle" ] && continue
    # here-string (not a pipe) so grep -q cannot SIGPIPE the producer
    if grep -qiF -- "$needle" <<< "$COMMIT_MSGS"; then
      fail "forbidden string '$needle' found in commit message history"
      any_history_failure=1
    fi
  done < <(read_patterns "$FORBID_FILE")
  if [ "$any_history_failure" = "0" ]; then
    pass "no forbidden strings in commit messages"
  fi
fi
...
if command -v gitleaks >/dev/null 2>&1; then
  ...
  if git archive HEAD | tar -x -C "$GL_DIR"; then
    ...
    if [ -f "$REPO_ROOT/.gitleaks.toml" ]; then
      GL_CONFIG_ARGS=(--config "$REPO_ROOT/.gitleaks.toml")
    fi
    if NO_COLOR=1 gitleaks detect --source "$GL_DIR" --no-banner --redact --no-git "${GL_CONFIG_ARGS[@]}" >"$GL_OUT" 2>&1; then
...
elif [ -d apps/vulscan-cli ]; then
  BUILD_OUT=$(mktemp)
  if (cd apps/vulscan-cli && go build ./... >"$BUILD_OUT" 2>&1); then
    pass "apps/vulscan-cli builds cleanly"
  else
    fail "apps/vulscan-cli build failed:"
    sed 's/^/        /' "$BUILD_OUT"
  fi
  rm -f "$BUILD_OUT"
else
  fail "apps/vulscan-cli not found — adjust check-public-release.sh"
fi
