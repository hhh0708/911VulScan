...
**NEVER run `make install` in `apps/vulscan-cli/`** — it overwrites the symlink with a copy.

The system uses a symlink: `/usr/local/bin/vulscan` → `apps/vulscan-cli/bin/vulscan`

To rebuild the Go CLI:
```bash
cd apps/vulscan-cli && go build -o bin/vulscan .
```
...
This is 911VulScan, a two-stage SAST tool using Claude for vulnerability analysis. Supports Python, JavaScript/TypeScript, and Go codebases with 4-level cost optimization.
...
911VulScan uses application context to understand what type of application it's analyzing, which dramatically reduces false positives by understanding what behaviors are intentional vs vulnerable.
...
**Manual override:** Create `911VULSCAN.md` or `911VULSCAN.json` in repo root. See `context/911VULSCAN_TEMPLATE.md` for format.

**Unsupported types:** If a repository doesn't match supported types, 911VulScan exits with error code 2 and instructions for creating a manual override.
