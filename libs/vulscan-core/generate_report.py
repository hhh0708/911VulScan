#!/usr/bin/env python3
"""
Generate interactive HTML security report from 911VulScan experiment results.
...
from core.verdict import is_reportable
from utilities.llm_pricing import format_cost, get_active_currency
from utilities.llm_config import resolve_llm_config
...
# Load environment variables from .env file (CWD .env cannot inject secrets)
load_scan_safe_dotenv()


REPORT_MODEL = model_for(ModelRole.SECONDARY)
MAX_TOKENS = 4096
...
def generate_remediation_guidance(findings: list) -> str:
    ...
    actionable = [f for f in findings if is_reportable(f['verdict'])]
...
"""
    ...
    # LLM output is embedded unescaped into the HTML report and the prompt
    # carries attacker-influenced finding text, so confine it to a safe
    # tag/attribute allowlist to prevent stored XSS.
    from utilities.html_sanitize import sanitize_report_html
    return sanitize_report_html(response.content[0].text)
...
def _build_pipeline_costs_html(step_reports: list[dict]) -> str:
    ...
    for sr in sorted(step_reports, key=lambda s: s.get("timestamp", "")):
        ...
        cost_str = format_cost(cost, sr.get("cost_currency", currency)) if cost > 0 else "-"
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr style="border-top: 2px solid var(--border); font-weight: bold">
                        <td>Total</td>
                        <td>{total_dur_str}</td>
                        <td>{format_cost(total_cost, currency)}</td>
                        <td></td>
                    </tr>
                </tbody>
            </table>
...
<body>
    <div class="container">
        ...
        <footer>
            <p>Generated by 911VulScan Security Analysis Tool</p>
        </footer>
    </div>
