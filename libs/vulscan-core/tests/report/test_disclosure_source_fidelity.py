        ...
        "results": [
            {
                ...
                "stage2_verification": {
                    ...
                    "decision": "confirmed",
                    "evidence_ids": ["ev_ping"],
                    "evidence": [
                        {
                            "evidence_id": "ev_ping",
                            "kind": "target_code",
                            "content": {"code": PING_CODE},
                        }
                    ],
                },
            },
...
def test_pipeline_output_carries_vulnerable_code_section(pipeline_output: dict):
    ...
    for finding in findings:
        ...
        # Markdown heading + fenced code (avoid locale-sensitive Chinese asserts on Windows)
        assert section.startswith("## "), f"missing markdown heading: {section[:40]!r}"
...
def test_splice_replaces_fabricated_code():
    ...
    # Real section appears exactly once (English legacy or Chinese heading).
    assert result.count("## Vulnerable Code") + result.count("## 漏洞代码") == 1
...
def test_splice_inserts_when_no_code_section():
    ...
    for marker in ("## Vulnerable Code", "## 漏洞代码"):
        if marker in result:
            code_pos = result.index(marker)
            break
    else:
        raise AssertionError("code section heading missing")
