...
def analyze_and_resolve(repo_path, files):
    ...
    return json.loads(resolved_out.read_text(encoding="utf-8"))
...
class TestConstructorDepsExtraction:
    ...
    def test_extracts_constructor_deps(self, nestjs_repo):
        ...
        data = json.loads(analyzer_out.read_text(encoding="utf-8"))
...
class TestDIAwareCallResolution:
    ...
    def test_versioned_implementation_without_type_link_is_unresolved(
        self, nestjs_repo_versioned
    ):
        """Prefix-name guessing is forbidden: CallService ≠ CallServiceV2.

        Without an exact type match or implements/extends link, the edge must
        stay unresolved rather than guessing the versioned class.
        """
        ...
        call_graph = data.get("callGraph") or data.get("call_graph") or {}
        resolver_calls = None
        resolver_id = None
        for fid, calls in call_graph.items():
            if "CallResolver.getCall" in fid:
        ...
        ), f"Must not guess versioned impl via name prefix, got: {resolver_calls}"

        unresolved = data.get("unresolved_edges") or data.get("unresolvedEdges") or []
        assert any(
            (e.get("caller") == resolver_id or "CallResolver.getCall" in (e.get("caller") or ""))
            and "getById" in (e.get("callee_name") or "")
            for e in unresolved
        ), f"Expected unresolved getById edge, got: {unresolved}"
