...
A user who runs `vulscan scan --verify` with an invalid API key should see
...
def all_errors_result() -> ScanResult:
    ...
    result.metrics = AnalysisMetrics(total_units=7, stage1_errors=7)
...
def normal_result() -> ScanResult:
    ...
    result = ScanResult(output_dir="/tmp/fake")
    result.metrics = AnalysisMetrics(total_units=7, stage2_confirmed=6, stage2_rejected=1)
    result.usage = UsageInfo(
        total_calls=7,
