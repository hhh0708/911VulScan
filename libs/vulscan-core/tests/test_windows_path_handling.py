...
These tests cover three fixes that prevent 911VulScan from running correctly on
Windows.
...
def pipeline_module(request, monkeypatch):
    ...
    try:
        if request.param == "javascript":
            ...
            mod_name = "vulscan_test_js_pipeline_cp1252"
        else:
            path = parsers_root / "go" / "test_pipeline.py"
            mod_name = "vulscan_test_go_pipeline_cp1252"
...
@pytest.mark.parametrize(
    "mod_name,rel_path",
    [
        ("vulscan_test_js_pipeline_utf8", "javascript/test_pipeline.py"),
        ("vulscan_test_go_pipeline_utf8", "go/test_pipeline.py"),
    ],
)
