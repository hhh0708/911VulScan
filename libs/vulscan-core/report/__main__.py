...
def cmd_summary(args):
    ...
    print(f"  Cost: {format_cost(usage['cost_usd'], currency)} ({usage['total_tokens']:,} tokens)")
...
def cmd_disclosures(args):
    ...
    for i, finding in enumerate(pipeline_data["findings"], 1):
        if not is_testable_finding(finding):
            continue
...
def main():
    parser = argparse.ArgumentParser(
        prog="report",
        description="Generate security reports and disclosure documents from 911VulScan pipeline output."
    )
