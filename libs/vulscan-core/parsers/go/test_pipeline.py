...
4. go_parser generate - Creates 911VulScan dataset format
...
from utilities.build_safety import ensure_safe_codeql_create
...
def _go_parser_runnable(path: str) -> bool:
    """True when the go_parser binary exists and actually runs on this platform.

    The repository ships a Windows PE binary; on Linux/macOS checkouts it
    exists but lacks the exec bit and cannot execute, so existence alone is
    not sufficient — probe execution and rebuild on any failure.
    """
    ...
    if sys.platform != "win32" and not os.access(path, os.X_OK):
        return False
    try:
        run_utf8([path, "--help"], capture_output=True, text=True, timeout=30)
...
class GoPipelineTest:
    def __init__(
        ...
        # Go parser binary location (Windows binaries carry the .exe suffix)
        go_parser_name = 'go_parser.exe' if sys.platform == 'win32' else 'go_parser'
        self.go_parser = os.path.join(self.parser_dir, 'go_parser', go_parser_name)
    ...
    def setup(self):
        ...
        # Build the Go parser when the binary is missing or cannot run on
        # this platform (the repo ships a Windows PE binary, which exists
        # but is not executable on Linux/macOS checkouts).
        if not _go_parser_runnable(self.go_parser):
            print("Building Go parser...")
            go_parser_dir = os.path.join(self.parser_dir, 'go_parser')
            result = run_utf8(
                ['go', 'build', '-o', os.path.basename(self.go_parser), '.'],
                cwd=go_parser_dir,
                capture_output=True,
                text=True,
    ...
    def run_go_parser_all(self) -> bool:
        ...
        if (
            ...
            try:
                ...
                # Normalise to camelCase for call-graph schema compatibility.
                ...
                native_cg_file = os.path.join(self.output_dir, 'call_graph_native.json')
                unresolved = []
                resolved = []
                native_forward = {}
                native_reverse = {}
                try:
                    cg_cmd = [
                        self.go_parser, 'callgraph',
                        '--output', native_cg_file,
                        self.repo_path,
                    ]
                    if self.skip_tests:
                        cg_cmd.insert(-1, '--skip-tests')
                    cg_run = run_utf8(cg_cmd, capture_output=True, text=True, timeout=120)
                    if cg_run.returncode == 0 and os.path.exists(native_cg_file):
                        native = read_json(native_cg_file)
                        unresolved = native.get("unresolved_edges") or []
                        resolved = native.get("resolved_edges") or []
                        native_forward = native.get("call_graph") or {}
                        native_reverse = native.get("reverse_call_graph") or {}
                except Exception:
                    pass
                ...
                raw_cg = {
                    ...
                    "call_graph": native_forward or call_graph,
                    "reverse_call_graph": native_reverse or reverse_call_graph,
                    "resolved_edges": resolved,
                    "unresolved_edges": unresolved,
                    "provenance": {
                        "builder": "parsers.go.test_pipeline",
                        "language": "go",
                    },
                }
                try:
                    from utilities.call_graph.schema import write_call_graph
                    write_call_graph(call_graph_file, raw_cg, language="go")
                except Exception:
                    write_json(call_graph_file, raw_cg)
    ...
    def apply_reachability_filter(self) -> bool:
        ...
        try:
            ...
            level = (
                ...
                else str(self.processing_level)
            )
            dataset = _canonical_reachability_filter(dataset, self.output_dir, level)
            meta = dataset.get("metadata", {}).get("reachability_filter", {})
            self.entry_points = set()
            self.reachable_units = {
                u.get("id")
                for u in dataset.get("units", [])
                if u.get("reachability") == "reachable"
            }
            for u in dataset.get("units", []):
                if u.get("is_entry_point"):
                    self.entry_points.add(u.get("id"))
            write_json(self.dataset_file, dataset)

            elapsed = (datetime.now() - start_time).total_seconds()
            summary = {
                "original_units": meta.get("original_units", 0),
                "entry_points": len(self.entry_points),
                "reachable_units": meta.get("kept_units", len(dataset.get("units", []))),
                "reduction_percentage": meta.get("reduction_percentage", 0),
            }
            ...
            print(f"{SYM_OK} Success ({elapsed:.2f}s)")
            print(f"  Structural roots: {len(self.entry_points)}")
            print(
                f"  Units: {summary['original_units']} {SYM_ARROW} "
                f"{summary['reachable_units']} ({summary['reduction_percentage']}% reduction)"
            )
            print()
    ...
    def run_codeql_analysis(self) -> bool:
        ...
        try:
            ...
            # Avoid CodeQL autobuild executing the target repo's build on the
            # host (RCE for untrusted code) unless explicitly opted-in.
            create_db_cmd = ensure_safe_codeql_create(create_db_cmd, language)
    ...
    def apply_exploitable_filter(self) -> bool:
        """No-op: Enhance no longer emits security_classification."""
        if not self.dataset_file or not os.path.exists(self.dataset_file):
            print("No dataset to filter")
            return False

        print("=" * 60)
        print("STAGE: exploitable_filter (no-op)")
        print("=" * 60)
        print()
        print(
            "Enhance is a neutral context layer; security_classification was removed. "
            "Keeping all units from prior filters."
        )
        print()
        ...
        try:
            ...
            classification_counts = {"neutral_context": len(units)}
            ...
            dataset["metadata"]["exploitable_filter"] = {
                ...
                "skipped": True,
                "reason": "enhancement_no_longer_classifies",
                "filtered_out": original_count - len(filtered_units),
    ...
    def run_full_pipeline(self):
        ...
        if self.enable_llm:
            ...
            if self.processing_level == ProcessingLevel.EXPLOITABLE and self.agentic:
