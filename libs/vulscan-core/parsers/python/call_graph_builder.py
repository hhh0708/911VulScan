...
class CallGraphBuilder:
    ...
    def __init__(self, extractor_output: Dict, options: Optional[Dict] = None):
        ...
        # caller -> set of callees discovered only via regex/fallback (low confidence)
        self.low_confidence_edges: Dict[str, Set[str]] = {}
    ...
    def _infer_local_types(self, tree: ast.AST, caller_file: str) -> Dict[str, Set[str]]:
        """Infer variable -> candidate class types from assignments and annotations.

        Rebinding accumulates a union of candidate types (never guessed unique).
        """
        types: Dict[str, Set[str]] = {}

        def _add(name: str, type_name: str) -> None:
            types.setdefault(name, set()).add(type_name)

        for node in ast.walk(tree):
            if isinstance(node, ast.AnnAssign) and isinstance(node.target, ast.Name):
                if isinstance(node.annotation, ast.Name):
                    _add(node.target.id, node.annotation.id)
                if isinstance(node.value, ast.Call) and isinstance(node.value.func, ast.Name):
                    _add(node.target.id, node.value.func.id)
    ...
    def _extract_calls_from_code(self, code: str, caller_id: str) -> Set[str]:
        ...
        try:
            ...
            # Regex/fallback must not produce exact edges.
            self._extract_calls_regex_as_unresolved(code, caller_id)
            return set()

        local_types = self._infer_local_types(tree, caller_file)

        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                resolved = self._resolve_call_node(
                    node, caller_file, caller_class, caller_id, local_types
                )
                if resolved:
                    calls.add(resolved)
    ...
    def _resolve_call_node(
        ...
        if isinstance(func, ast.Name):
            ...
            if self._is_builtin(func_name):
                return None
            return self._resolve_simple_call(func_name, caller_file, caller_id)
        ...
        # Method call: obj.method(...)
            ...
            if isinstance(obj, ast.Name) and obj.id == 'self':
                ...
                self._record_unresolved(caller_id, f"self.{method_name}", "no_type")
                return None
            ...
            if isinstance(obj, ast.Name) and obj.id == 'cls':
                if caller_class:
                    return self._resolve_self_call(method_name, caller_file, caller_class)
                self._record_unresolved(caller_id, f"cls.{method_name}", "no_type")
                return None

            # super().method(...)
            if isinstance(obj, ast.Call) and isinstance(obj.func, ast.Name) and obj.func.id == 'super':
                self._record_unresolved(caller_id, f"super().{method_name}", "no_type")
                return None

            # Typed receiver: use inferred type union, not variable name.
            if isinstance(obj, ast.Name):
                obj_name = obj.id
                # Import/module attribute call (scope-sensitive).
                file_imports = self.imports.get(caller_file, {})
                if obj_name in file_imports:
                    return self._resolve_module_call(obj_name, method_name, caller_file, caller_id)

                type_names = local_types.get(obj_name, set())
                if not type_names:
                    # Do not treat variable name as a class name.
                    if self._is_common_method(method_name):
                        return None
                    self._record_unresolved(
                        caller_id, f"{obj_name}.{method_name}", "no_type"
                    )
                    return None

                matches: List[str] = []
                for type_name in type_names:
                    class_key = f"{caller_file}:{type_name}"
                    ...
                    # Also search imported class types by name in other files via imports.
                    if type_name in file_imports:
                        resolved = self._resolve_import(
                            file_imports[type_name], method_name, caller_file
                        )
                        if resolved:
                ...
                if len(matches) > 1:
                    ...
                    return None
                self._record_unresolved(
                    caller_id, f"{obj_name}.{method_name}", "no_type", list(type_names)
                )
                return None

            # Chained / dynamic receivers
            if self._is_common_method(method_name):
                return None
            self._record_unresolved(caller_id, method_name, "dynamic")
            return None

        # Indirect call: (fn)(...) / getattr / etc.
        self._record_unresolved(caller_id, "<callable>", "dynamic")
        return None

    def _resolve_simple_call(
        self, func_name: str, caller_file: str, caller_id: Optional[str] = None
    ) -> Optional[str]:
        """Resolve a simple function call (scope-sensitive; no global unique guess)."""
        # 1. Check same file first (non-method, prefer non-nested)
        ...
        if len(same_file_matches) > 1:
            # Prefer non-nested; if still ambiguous, unresolved.
            top = [f for f in same_file_matches if not self.functions[f].get('parent_id')]
            if len(top) == 1:
                return top[0]
            if caller_id:
                self._record_unresolved(caller_id, func_name, "ambiguous", same_file_matches)
            return None

        # 2. Check imports and resolve (alias-sensitive)
        file_imports = self.imports.get(caller_file, {})
        if func_name in file_imports:
            import_path = file_imports[func_name]
            resolved = self._resolve_import(import_path, func_name, caller_file)
            if resolved:
                return resolved
            candidates = [
                c for c in self.functions_by_name.get(func_name, [])
                if not self.functions.get(c, {}).get('class_name')
            ]
            if caller_id:
                self._record_unresolved(
                    caller_id, func_name, "import_failed", candidates
                )
            return None

        # 3. No repo-wide unique-name linking — leave unresolved.
        candidates = [
            c for c in self.functions_by_name.get(func_name, [])
            if not self.functions.get(c, {}).get('class_name')
        ]
        if candidates and caller_id:
            ...
            self._record_unresolved(caller_id, func_name, reason, candidates)
        elif caller_id:
            self._record_unresolved(caller_id, func_name, "unresolved")
        return None
    ...
    def _resolve_module_call(
        ...
        """Resolve a module.function() call via import alias (not variable-as-type)."""
        ...
        if obj_name in file_imports:
            ...
            resolved = self._resolve_import(import_path, method_name, caller_file)
            if resolved:
                return resolved
            if caller_id:
                self._record_unresolved(
                    caller_id, f"{obj_name}.{method_name}", "import_failed"
                )
            return None

        # ClassName.method only when obj_name is a known class in this file
        # (static type / class object), not an instance variable name.
        class_key = f"{caller_file}:{obj_name}"
        if class_key in self.methods_by_class:
            matches = [
                fid
                for fid in self.methods_by_class[class_key]
                if self.functions.get(fid, {}).get('name') == method_name
            ]
            if len(matches) == 1:
                return matches[0]
            if caller_id:
                self._record_unresolved(
                    caller_id,
                    f"{obj_name}.{method_name}",
                    "ambiguous" if matches else "no_type",
                    matches,
                )
            return None

        if caller_id:
            self._record_unresolved(caller_id, f"{obj_name}.{method_name}", "unresolved")
        return None
    ...
    def _extract_calls_regex_as_unresolved(self, code: str, caller_id: str) -> None:
        """Regex/fallback extraction: unresolved or low-confidence only (never exact)."""
        caller_file = caller_id.split(':')[0]
        ...
        for match in re.finditer(pattern, code):
            ...
            if self._is_builtin(func_name):
                continue
            # Same-file scoped candidates only — never repo-wide unique guess.
            same_file = [
                fid
                for fid in self.functions_by_file.get(caller_file, [])
                if self.functions.get(fid, {}).get('name') == func_name
                and fid != caller_id
            ]
            if len(same_file) == 1:
    ...
    def build_call_graph(self) -> None:
        ...
        for func_id, func_data in self.functions.items():
            ...
            # Merge regex/fallback low-confidence callees into adjacency (not exact).
    ...
    def export(self) -> Dict:
        ...
        raw = {
            ...
            'provenance': {
                'builder': 'parsers.python.call_graph_builder',
                'language': 'python',
            },
        }
        try:
            from utilities.call_graph.schema import finalize_call_graph, to_legacy_export
            canonical = finalize_call_graph(raw, language='python')
            legacy = to_legacy_export(canonical)
            # In-memory hybrid for UnitGenerator; file writes use write_call_graph.
            return {
                **canonical,
                **legacy,
                'classes': self.classes,
