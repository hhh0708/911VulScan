...
class FunctionExtractor:
    ...
    def read_file(self, file_path: Path) -> str:
        """Read and cache file contents (cache bounded against huge repos)."""
        path_str = str(file_path)
        if path_str not in self.file_cache:
            if len(self.file_cache) >= self.file_cache_limit:
    ...
    def process_function(
        ...
        force_unit_type: Optional[str] = None,
    ) -> Tuple[str, Dict]:
        """Process a function definition and extract metadata."""
        func_name = node.name
        if parent_qualified:
            qualified_name = f"{parent_qualified}.{func_name}"
        elif class_name:
        ...
        if force_unit_type:
            unit_type = force_unit_type
        elif parent_id:
            unit_type = 'nested_function'
        else:
        ...
        func_data = {
            ...
            'unit_type': unit_type,
            'parent_id': parent_id,
            'is_exported': False if parent_id else None,
        }
        if func_name == '__call__' and class_name:
            func_data['unit_type'] = 'callable'
            func_data['kind'] = 'callable'

        return func_id, func_data

    def _extract_nested_callables(
        self,
        node: ast.AST,
        file_path: str,
        content: str,
        ...
        """Extract nested functions and lambdas under a parent callable."""
        relative_path = str(Path(file_path).relative_to(self.repo_path))
        for child in ast.walk(node):
            if child is node:
                continue
            if isinstance(child, (ast.FunctionDef, ast.AsyncFunctionDef)):
                # Only direct nesting relative to this parent's body walk —
                # skip deeper nodes that will be picked up from their own parent.
                # Use parent body membership check via lineno containment.
                pass

        # Direct body nested defs only (avoid double-counting via walk).
        body = getattr(node, 'body', []) or []
        for item in body:
            if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                func_id, func_data = self.process_function(
                    item,
                    file_path,
                    content,
                    class_name=class_name,
                    ...
                    parent_id=parent_id,
                    force_unit_type='nested_function',
                )
                if func_id not in self.functions:
                    self.functions[func_id] = func_data
                    self.stats['total_functions'] += 1
                    self.stats['by_type']['nested_function'] = (
                        self.stats['by_type'].get('nested_function', 0) + 1
                    )
                    self._extract_nested_callables(
                        item, file_path, content, func_data['qualified_name'], func_id, class_name
                    )
            elif isinstance(item, ast.Assign):
                for target in item.targets:
                    if isinstance(target, ast.Name) and isinstance(item.value, ast.Lambda):
                        lam_name = target.id
                        qn = f"{parent_qualified}.{lam_name}"
                        func_id = f"{relative_path}:{qn}"
                        if func_id not in self.functions:
                            self.functions[func_id] = {
                                'name': lam_name,
                                'qualified_name': qn,
                                'file_path': relative_path,
                                'start_line': item.value.lineno,
                                'end_line': getattr(item.value, 'end_lineno', item.value.lineno),
                                'code': self.get_source_segment(content, item) or 'lambda',
                                'class_name': class_name,
                                'decorators': [],
                                'is_async': False,
                                'parameters': [
                                    a.arg for a in item.value.args.args
                                ],
                                'docstring': None,
                                'unit_type': 'lambda',
                                'kind': 'lambda',
                            ...
                            self.stats['total_functions'] += 1
                            self.stats['by_type']['lambda'] = (
                                self.stats['by_type'].get('lambda', 0) + 1
                            )
            # Decorators wrapping: record decorator names on parent already;
            # nested defs inside decorator factories are handled when those
            # factories are processed as top-level/nested functions.
    ...
    def process_file(self, file_path: Path) -> None:
        ...
        try:
            # Never follow symlinks (untrusted repo: external targets would
            # leak into LLM prompts); skip oversized files.
            if file_path.is_symlink() or file_path.stat().st_size > self.max_file_size:
                return
        except OSError:
        ...
        for node in ast.iter_child_nodes(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                self._extract_nested_callables(
                    node, file_path, content, func_data['qualified_name'], func_id
                )
    ...
    def extract_all(self, files: Optional[List[str]] = None) -> Dict:
        ...
        if files:
            ...
            # Scan all .py files. os.walk never descends into directory
            # symlinks (untrusted repo: loops / external targets), and
            # process_file skips symlinked or oversized files.
            for root, dirs, names in os.walk(self.repo_path, followlinks=False):
                try:
