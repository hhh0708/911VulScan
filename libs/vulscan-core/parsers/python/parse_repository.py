...
datasets for 911VulScan's vulnerability analysis. It orchestrates a
4-stage pipeline:
...
def generate_analyzer_output(extractor_result: dict) -> dict:
    ...
    This format is compatible with 911VulScan's RepositoryIndex class,
...
def parse_repository(repo_path: str, options: dict = None) -> tuple:
    ...
    Returns:
        Tuple of (dataset, analyzer_output):
            - dataset: Dictionary compatible with 911VulScan experiment.py
    ...
    if output_dir:
        ...
        try:
            ...
            # Write canonical-only file; keep in-memory hybrid for UnitGenerator.
            written = write_call_graph(graph_file, call_graph_result, language='python')
            unresolved = (written.get('provenance') or {}).get('statistics', {}).get(
                'unresolved'
            ) or {}
        except Exception:
        ...
        print(
            f"  Unresolved edges: {unresolved.get('total', len(call_graph_result.get('unresolved_edges', [])))}",
            file=sys.stderr,
        )
