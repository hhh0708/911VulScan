...
The 911VulScan parser pipeline transforms JavaScript/TypeScript repositories into structured datasets for security analysis. It supports four processing levels with progressive cost optimization through filtering.
...
│  Output: dataset.json (911VulScan dataset format)                          │
│                                                                          │
├─────────────────────────────────────────────────────────────────────────┤
...
**Purpose**: Create 911VulScan dataset from analyzer output. Includes DependencyResolver for call graph building. Outputs standard 911VulScan format with assembled dependency code.

**Input**:
- `analyzer_output.json` from Stage 2
- Optional: `routes.json` for route handler tagging

**Output** (`dataset.json`) — Standard 911VulScan Format:
```json
{
...
**Key Format Requirements** (for 911VulScan compatibility):
...
| `validate_dataset_schema.py` | `vulscan/` | Validate dataset matches 911VulScan schema |
...
### Node.js
```bash
cd /path/to/vulscan/parsers/javascript
...
- **2025-12-25**: Fixed unit_generator.js to output standard 911VulScan format (`enhanced`, `files_included`, assembled `primary_code` with file boundaries). Added `validate_dataset_schema.py`.
