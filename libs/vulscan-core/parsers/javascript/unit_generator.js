...
class UnitGenerator {
    ...
    _createUnit(functionId, funcData, routeHandlerMap) {
        ...
        // Assemble enhanced code with dependencies (911vulscan standard format)
...
if (require.main === module) {
    ...
    try {
        ...
        if (outputFile) {
            ...
            // Write resolved call_graph.json after dependency resolution (not the
            // unresolved analyzer stub).
            if (generator.resolver) {
                const cgPath = path.join(path.dirname(outputFile), 'call_graph.json');
                const cgExport = generator.resolver.export();
                fs.writeFileSync(cgPath, JSON.stringify(cgExport, null, 2));
                console.error(`Call graph written to: ${cgPath}`);
                console.error(`Unresolved edges: ${(cgExport.unresolved_edges || []).length}`);
            }
        } else {
            console.log(output);
        }
