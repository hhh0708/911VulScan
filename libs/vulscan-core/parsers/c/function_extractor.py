...
class FunctionExtractor:
    ...
    def process_file(self, file_path: Path, relative_path: Optional[str] = None) -> None:
        ...
        if relative_path is None:
            try:
                ...
                relative_path = f"__vulscan_extra__/{file_path.name}"
    ...
    def extract_from_scan(self, scan_result: Dict) -> Dict:
        ...
        for file_info in scan_result.get('files', []):
            ...
            if abs_path:
                self.process_file(Path(abs_path), relative_path=rel)
                continue
            file_path = self.repo_path / rel
            if file_path.exists():
                self.process_file(file_path, relative_path=rel)
