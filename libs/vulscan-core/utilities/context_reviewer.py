...
class ContextReviewer:
    ...
    def review_context(
        ...
        try:
            ...
            review = self.client.analyze_json_sync(
                prompt,
                model=resolve_model(
                    getattr(self.client, "model", model_for(ModelRole.SECONDARY)),
                    getattr(self.client, "_config", None),
                ),
                context=f"context review {route}",
