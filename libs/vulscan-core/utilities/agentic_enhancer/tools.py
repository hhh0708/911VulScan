...
TOOL_DEFINITIONS = [
    ...
    },
    {
        "name": "finish",
        "description": (
            "Complete context gathering. Return ONLY the neutral Enhancement "
            "Schema fields. Do not classify exploitability or invent callers."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "related_units": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            "id": {
                                "type": "string",
                                "description": "Related unit / function ID",
                            },
                            "relation_type": {
                                "type": "string",
                                "description": (
                                    "caller | callee | type_definition | "
                                    "import | other"
                                ),
                            },
                            "reason": {
                                "type": "string",
                                "description": "Why this unit is related",
                            },
                        },
                        "required": ["id", "relation_type", "reason"],
                    },
                    "description": "Related units evidenced by code or tools",
                },
                "relation_type": {
                    "type": "string",
                    "description": "Primary relation summary for this unit",
                },
                "types_and_definitions": {
                    "type": "array",
                    "description": "Type/definition observations",
                    "items": {"type": "object"},
                },
                "call_context": {
                    "type": "object",
                    "description": "Observed call context (not new graph edges)",
                    "properties": {
                        "direct_calls": {
                            "type": "array",
                            "items": {"type": "string"},
                ...
                "dataflow_observations": {
                    "type": "array",
                    "description": "Neutral dataflow facts (no vuln labels)",
                    "items": {"type": "object"},
                },
                "build_runtime_context": {
                    "type": "object",
                    "description": "Build/runtime/config observations",
                },
                "unknowns": {
                    "type": "array",
                    "description": "Missing evidence / unresolved questions",
                    "items": {"type": "object"},
                },
            },
            "required": [
...
class ToolExecutor:
    ...
    def _finish(self, input: dict) -> dict:
        """Validate finish payload against the neutral Enhancement Schema."""
        required = [
            "related_units",
            "call_context",
            "dataflow_observations",
        ...
        if not isinstance(input.get("related_units"), list):
            return {"error": "related_units must be an array"}
        if not isinstance(input.get("call_context"), dict):
            return {"error": "call_context must be an object"}
        if not isinstance(input.get("dataflow_observations"), list):
            return {"error": "dataflow_observations must be an array"}
        if not isinstance(input.get("unknowns"), list):
            return {"error": "unknowns must be an array"}

        # Reject security-classification / verdict fields if the model emits them.
        forbidden = {
        ...
        bad = forbidden & set(input.keys())
        if bad:
            return {"error": f"Forbidden fields in finish payload: {sorted(bad)}"}
