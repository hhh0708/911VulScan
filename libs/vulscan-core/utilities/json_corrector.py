...
def extract_json_with_llm(
    ...
    try:
        ...
        extraction_model = resolve_model(
            getattr(client, "model", model_for(ModelRole.SECONDARY)),
            getattr(client, "_config", None),
        )
        return client.analyze_json_sync(
            prompt,
            model=extraction_model,
            max_tokens=2048,
            context="JSON extraction correction",
