...
class GroundTruthChallenger:
    ...
    def __init__(self, client: AnthropicClient, model: str = model_for(ModelRole.PRIMARY)):
        """
    ...
    def challenge_false_positive(
        ...
        try:
            ...
            parsed = self.client.analyze_json_sync(
                prompt,
                model=self.model,
                context=f"FP challenge {route_key}",
    ...
    def challenge_false_negative(
        ...
        try:
            ...
            parsed = self.client.analyze_json_sync(
                prompt,
                model=self.model,
                context=f"FN challenge {route_key}",
                retries=DEFAULT_JSON_RETRIES,
            )

            if not parsed:
                print(f"      Failed to parse FN challenge response for {route_key}", file=sys.stderr)
