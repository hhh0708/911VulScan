...
def parse_missing_context_with_llm(
    ...
    try:
        ...
        parsed = client.analyze_json_sync(
            prompt,
            model=resolve_model(getattr(client, "model", model_for(ModelRole.SECONDARY)), getattr(client, "_config", None)),
            context="missing context extraction",
...
def search_files_for_context(
    ...
    for i, batch in enumerate(batches):
        ...
        try:
            ...
            result = client.analyze_json_sync(
                prompt,
                model=resolve_model(getattr(client, "model", model_for(ModelRole.SECONDARY)), getattr(client, "_config", None)),
                context=f"file search batch {i + 1}/{len(batches)}",
                retries=DEFAULT_JSON_RETRIES,
            )
    ...
    def attempt_correction(
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                payload = self.client.analyze_json_sync(
                    prompt,
                    context="context correction re-analysis",
                    retries=DEFAULT_JSON_RETRIES,
                )
                ...
                new_result = self._normalize_result(payload)
...
def test_corrector():
    ...
    for i, test_case in enumerate(test_cases):
        ...
        # Test file gathering
        repo_path = os.path.expanduser("~/code/dvna")
