...
| `build_plan.py` | Build models | `BuildPlan`, `ResourceScope`, `RuntimePolicy` |
| `language_registry.py` | Staging registry | `materialize_build_plan()`, `should_use_framework_dockerfile()` |
| `python_stage.py` / `javascript_stage.py` | Project staging | bounded copy of package trees |
| `compose_builder.py` | Compose topology | `build_compose_yaml()` — internal network, cap_drop |
| `dockerfile_builder.py` | Dockerfile assembly | `finalize_dockerfile()`, native/go/python/js builders |
| `docker_executor.py` | Container exec | `run_single_container(batch_run_id=...)`, label-scoped cleanup |
...
For each finding:
  1. test_generator.generate_test(finding, repo_info, tracker)
     → Claude Sonnet generates: test_script, requirements, needs_attacker_server
     → LLM dockerfile/docker_compose ignored; placeholder stored for schema compat
     → Cost: ~$0.03-0.05 per finding

  2. docker_executor.run_single_container(generation, finding_id, batch_run_id=...)
     → materialize BuildPlan → stage source tree → finalize Dockerfile once at build time
     → Single container: docker build + docker run (--network none, cap_drop ALL, labels)
     → Multi-service: framework compose (internal network, test + attacker)
     → Cleanup scoped to batch run_id labels (no global vulscan-test-* sweep)
     → Timeout: 120s container, 300s build; timed-out containers explicitly removed
...
## Dockerfile Assembly (C/C++)

For C/C++ findings with staged source files, **the framework owns the Dockerfile**. The LLM focuses on `test_script`; `dockerfile_builder.build_native_dockerfile()` emits:

- `FROM gcc:13`, `WORKDIR /work`
- `COPY` for each staged file (source, headers, test harness)
- `RUN gcc|g++` with AddressSanitizer, linking translation units not `#include`d in the test
- `RUN test -x` and `CMD` for the binary under `/work`

This prevents LLM-generated shell heredocs (`RUN cat > file.h << 'EOF'`) from being parsed as invalid Dockerfile instructions (e.g. `unknown instruction: extern`).

Non-native languages still use LLM Dockerfiles, validated by `validate_dockerfile()` before acceptance.
...
- `utilities/llm_client.py` — `AnthropicClient`, `TokenTracker` (Sonnet model: `claude-opus-4-6`)
