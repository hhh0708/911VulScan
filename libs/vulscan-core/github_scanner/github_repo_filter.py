...
except ImportError:  # allow `python github_scanner/github_repo_filter.py`
...
def _make_session() -> requests.Session:
    ...
    s.headers.update({
        ...
        "User-Agent": "vulscan-repo-scanner/1.0",
...
def _short_error_message(resp: requests.Response) -> str:
    """Short, secret-free error detail from a GitHub response body."""
    try:
        msg = (resp.json() or {}).get("message", "")
    except Exception:
        msg = ""
    ...
    *,
    ...
    for _ in range(10):
        ...
        try:
            ...
            print(f"[github] GET {url} failed: {type(exc).__name__}")
            continue
        ...
        if resp.status_code >= 500:
            print(f"[github] GET {url} -> {resp.status_code} (server error, retrying)")
            continue

        if resp.status_code != 200:
            detail = _short_error_message(resp)
            print(f"[github] GET {url} -> {resp.status_code} {detail}".rstrip())
            return None

        try:
            return resp.json()
        except Exception:
            print(f"[github] GET {url} -> 200 but response is not valid JSON")
            return None

    print(f"[github] GET {url} -> giving up after 10 attempts")
    return None
