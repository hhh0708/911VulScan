...
This document defines the step-by-step protocol for security analysis of GitHub repositories using 911VulScan. Each step requires explicit approval before proceeding to the next.
...
cp codeql-results.sarif /path/to/vulscan/datasets/<repo>/
```
...
**Purpose:** Remove units already identified by CodeQL. 911VulScan focuses on finding vulnerabilities that CodeQL misses.
...
cp codeql-results.sarif ~/code/vulscan/datasets/<repo>/
