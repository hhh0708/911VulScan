...
def _fmt_cost(amount: float, tracker=None) -> str:
    """Format cost using the active provider currency."""
    from utilities.llm_pricing import format_cost, resolve_display_currency

    currency = None
    if tracker:
        currency = resolve_display_currency(tracker.get_totals().get("cost_currency"))
    else:
        currency = resolve_display_currency()
    return format_cost(amount, currency)
...
class ProgressReporter:
    ...
    def report(
        ...
        with self._lock:
            ...
            meta = f"(elapsed {_fmt_duration(elapsed)}, ETA {eta}, {_fmt_cost(unit_cost, self.tracker)})"
            parts.append(meta)
    ...
    def _print_summary(self, elapsed: float, cost: float) -> None:
        ...
        line = (
            ...
            f"ETA {eta} | "
            f"cost {_fmt_cost(cost, self.tracker)}"
            f" ---"
        )
        print(line, file=sys.stderr, flush=True)
