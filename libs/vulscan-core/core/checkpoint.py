...
class StepCheckpoint:
    ...
    def load_ids(self, skip_errors: bool = True) -> set[str]:
        ...
        for unit_id, data in loaded.items():
            if skip_errors:
                # Enhance: agent_context.error (legacy) or enhancement.provenance.error
                agent_ctx = data.get("agent_context", {})
                if agent_ctx.get("error"):
                    continue
                enh = data.get("enhancement") or {}
                if (enh.get("provenance") or {}).get("error"):
                    continue
    ...
    def status(checkpoint_dir: str) -> dict:
        ...
        calls this via ``python -m vulscan checkpoint-status`` instead of
        ...
        for filename in os.listdir(checkpoint_dir):
            ...
            # Enhance-style: agent_context.error (legacy) or enhancement.provenance.error
            agent_ctx = data.get("agent_context", {})
            enh = data.get("enhancement") or {}
            enh_err = (enh.get("provenance") or {}).get("error")
            if agent_ctx.get("error"):
                is_error = True
                err = agent_ctx["error"]
                ...
                is_error = True
                err_type = (
                    enh_err.get("type", "unknown")
                    if isinstance(enh_err, dict)
                    else "unknown"
                )
