...
def step_context(step: str, output_dir: str, inputs: dict | None = None):
    ...
    try:
        ...
        safe_msg = safe_exception_message(exc)
        report.status = "failed"
        report.errors.append(safe_msg)
        print(f"[{step}] ERROR: {safe_msg}", file=sys.stderr)
        ...
        report.cost_currency = resolve_display_currency()
        report.token_usage = {
        ...
        # Normalize legacy success → completed
        if report.status in ("success", "ok", ""):
            report.status = "completed"
        if report.status == "error":
            report.status = "failed"
        if report.errors and report.status == "completed":
            report.status = "partial"

        report.write(output_dir)
        print(
            f"[{step}] Report: {output_dir}/{step}.report.json "
            f"({report.duration_seconds}s, {format_cost(report.cost_usd, report.cost_currency)})",
            file=sys.stderr,
        )
