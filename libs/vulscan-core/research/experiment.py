#!/usr/bin/env python3
"""
911VulScan Experiment Runner - Two-Stage Vulnerability Analysis
...
from core.verdict import summarize_transition, classify_transition, is_actionable
...
# Repository paths for context correction (adjust CODE_DIR to your local checkouts)
CODE_DIR = os.path.expanduser("~/code")

REPO_PATHS = {
    "dvna": os.path.join(CODE_DIR, "dvna"),
    "nodegoat": os.path.join(CODE_DIR, "NodeGoat"),
    "juice_shop": os.path.join(CODE_DIR, "juice-shop"),
    "flowise": os.path.join(CODE_DIR, "test_repos/Flowise"),
    "geospatial": os.path.join(CODE_DIR, "test_repos/streamlit-geospatial"),
    "object_browser": os.path.join(CODE_DIR, "test_repos/object-browser"),
    "object_browser_vuln25": os.path.join(CODE_DIR, "test_repos/object-browser"),
    "uptime_kuma": os.path.join(CODE_DIR, "test_repos/uptime-kuma"),
    "code_server": os.path.join(CODE_DIR, "test_repos/code-server"),
    "code_server_vuln4": os.path.join(CODE_DIR, "test_repos/code-server"),
    "anything_llm": os.path.join(CODE_DIR, "test_repos/anything-llm"),
    "flowise_non_codeql": os.path.join(CODE_DIR, "test_repos/Flowise"),
    "flowise_stage1_vuln": os.path.join(CODE_DIR, "test_repos/Flowise"),
    "langchain": os.path.join(CODE_DIR, "test_repos/langchain"),
    "langchain_vuln": os.path.join(CODE_DIR, "test_repos/langchain"),
    "flask": os.path.join(CODE_DIR, "test_repos/flask"),
    "paperless": os.path.join(CODE_DIR, "test_repos/paperless-ngx"),
    "paperless_stage2": os.path.join(CODE_DIR, "test_repos/paperless-ngx"),
    "n8n": os.path.join(CODE_DIR, "test_repos/n8n"),
}
...
def analyze_unit(
    ...
    classification_reasoning = (
        ...
        or agent_context.get("reasoning")
    )
    ...
    # Resolve language once and feed the same value to deterministic facts and
    # the LLM. Falling back to a literal "code" silently disabled
    # language-specific security semantics.
    ...
    # Generate prompt from parsed/enhanced unit + app context only (no static heuristics).
    ...
    internal_vuln = security_classification == "vulnerable_internal"
    ...
    system_prompt = get_stage1_system_prompt(
        app_context=app_context,
        internal_vuln=internal_vuln,
    )
    try:
        payload = client.analyze_json_sync(
            prompt,
            system=system_prompt,
            context=f"stage1 analysis {route_key}",
    ...
    response = json.dumps(result) if result.get("verdict") != "ERROR" else ""

    # If parsing failed or verdict is missing, try JSON correction
    if result.get("verdict") in ("ERROR", None) and response:
...
def run_experiment(
    ...
    """
    # Select model: "opus" opts into the premium model, otherwise the standard
    # primary. Concrete ids live in utilities.model_registry.
    model_id = model_for(ModelRole.PREMIUM if model == "opus" else ModelRole.PRIMARY)
    print(f"Using model: {format_active_llm_label(model_id)}")
    ...
    if app_context:
        print(f"Application context status: {getattr(app_context, 'status', 'ok')}")
        if app_context.purpose:
    ...
    for i, unit in enumerate(units):
        ...
        try:
            ...
            if result.get("verdict") == "INSUFFICIENT_CONTEXT" and corrector:
                ...
                # Keep correction prompts on the same language/fact contract as
                # the primary Stage 1 path so retries do not silently drop
        ...
        if not analyzer_output_path or not os.path.exists(analyzer_output_path):
            ...
            for i, result in enumerate(valid_results):
                ...
                try:
                    ...
                    result["stage1_finding"] = stage1_finding
                    metrics["verifications_total"] += 1

                    # Count by the verdict transition, not the raw `agree` flag:
                    # a kept verdict (reasoning refined) is an agreement, only an
                    # actual verdict change is a disagreement.
                    transition = classify_transition(
                        stage1_finding,
                        verification.correct_finding,
                        reasoning_agrees=verification.agree,
                    )
                    if transition.verdict_changed:
                        metrics["verifications_disagreed"] += 1
                        result["finding"] = transition.final.value
                        result["verification_note"] = (
                            f"Changed from {transition.stage1.value} to {transition.final.value}"
                        )
                    else:
                        metrics["verifications_agreed"] += 1
                        if transition.reasoning_refined:
                            result["verification_note"] = "Verdict kept; Stage 2 refined the reasoning"

                    final_finding = result.get("finding", verification.correct_finding)
                    verification_state = result.get("verification", {})
                    # A kept verdict is a confirmation even when Stage 2 refined
                    # the reasoning; only an actual verdict change is a "change".
                    # (Separates the reasoning dimension from the verdict
                    # dimension, so "vulnerable -> vulnerable" can never print as
                    # a disagreement.)
                    symbol, label = summarize_transition(
                        stage1_finding,
                        final_finding,
                        reasoning_agrees=verification_state.get("agree"),
                    )
